...
{
	const int WAYS = 4; // ways for balance sort

	void Balanced_Multiway_Merge() //WAYS == 4
	{
		int k = 1;
	...
	{
		char **a = new char*[n]; // n строк
		for (int i = 0; i < n; i++)
		{
			a[i] = new char[10]; // Под каждую строку выделяем массив в 10 символов
			sprintf(a[i], "%s%d%s", "tmp", i, ".txt");
			ofstream N(a[i]); // создание файла tmp'i'.txt
			N.close();
		}
		for (int i = 0; i < n; i++)
		{
	...
	void Multiway_Distribution(int n, int k) // n = WAYS
	{
		ifstream A("Input.txt");
		ofstream B[WAYS];
		char **a = new char*[n]; // n == WAYS строк
		for (int i = 0; i < n; i++)
		{
			a[i] = new char[10];
			sprintf(a[i], "%s%d%s", "tmp", i, ".txt");
		...
		int flag = 0; // if first == 0, write data in tmp0.txt, == 1 in tmp1.txt ...
		int tmp;
		while (!A.eof())
		{
	...
	void Multiway_Merge(int k)
	{
		...
		ifstream B[WAYS];
		char **a = new char*[WAYS]; // n == WAYS строк
		...
		int position = 0,
			min = INT_MAX - 1;
		int b[WAYS],
			ib[WAYS]; // for saving data and counting
		bool marker,
			used[WAYS] = { false },
			limit[WAYS] = { false };
	...
	bool Check_one_element_less(int *M, int k) // WAYS elements in M
	{
		int counter = 0;
		for (int i = 0; i < WAYS; i++)
			counter = counter + M[i];
